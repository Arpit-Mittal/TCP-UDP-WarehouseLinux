...
#define MYPORT "13657"
#define SERVERPORT "17657"
#define MYPORT11 "15657"
#define SERVERPORT11 "19657"
#define STATICPORT11 "14657"
#define STATICPORT12 "16657"
...
//Phase 1: store_3 has TCP port number _____ and IP address _____
//Phase 1: The outlet vector<A,B,C> for store_3 has been sent to the central warehouse
//End of Phase 1 for store_3
//Phase 2: Store_3 has UDP port ______ and IP address ______
//Phase 2: Store_3 received the truck-vector<A,B,C> from store_2
//Phase 2: Store_3 has UDP port ______ and IP address ______
//Phase2: The updated truck-vector<A,B,C> has been sent to store_4
//Phase2: Store_3 updated outlet-vector is <A,B,C>
//Phase 2: Store_3 has UDP port ______ and IP address ______
//Phase2: truck-vector<A,B,C> has been received from Store_2
//Phase 2: Store_3 has UDP port ______ and IP address ______
//Phase2: The updated truck-vector<A,B,C> has been sent to store_4
//Phase2: Store_3 updated outlet vector is <A,B,C>
...
int main(int argc, char *argv[])
{
	...
	int rv6;
	int numbytes6;
	struct sockaddr_storage their_addr6;
	char buf66[MAXBUFLEN];
	socklen_t addr_len6;
...
static const char filename[] = "store3.txt";
...
/////////////////////////////////
  ...
  return -1;
		...
		printf("Phase 1: store_3 has TCP port number");
	...
	//End of Phase 1 for store_1
	 ...
	 if (send(sockfd, ca, (globalcounter+1), 0) == -1)
			...
			printf("for store_3 has been sent to the central warehouse\n");
	...
	//printf("listener: got packet from %s\n",
		inet_ntop(their_addr2.ss_family,
			...
			s2, sizeof s2);
		printf("Phase 2: Store_3 has UDP port");
		printf("%s",MYPORT);
		printf("and IP address");
		printf("%s",s2);
	...
	//printf("length");
	//printf("%d",(strlen(ca)));
	printf("Phase 2: Store_3 received the truck-vector");
	printf("%s",buf22);
	printf("from store_2");
...
for(j=0;j<(strlen(buffer31));j++)
   ...
   /* if (argc != 3) {
	...
	printf("%s",buffer41);
	printf(" has been sent to store_4");
	printf("Phase2: Store_3 updated outlet-vector is");
	printf("<");
	printf("%d",o1);
	printf(",");
	printf("%d",o2);
   ...
   // printf("talker: sent %d bytes to %s\n", numbytes3, "nunki.usc.edu");
	...
	hints6.ai_family = AF_UNSPEC; // set to AF_INET to force IPv4
	hints6.ai_socktype = SOCK_DGRAM;
	hints6.ai_flags = AI_PASSIVE; // use my IP

	if ((rv6 = getaddrinfo("nunki.usc.edu", MYPORT11, &hints6, &servinfo6)) != 0) {
		fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv6));
		return 1;
	}
	...
	for(p6 = servinfo6; p6 != NULL; p6 = p6->ai_next) {
		...
		if (bind(sockfd6, p6->ai_addr, p6->ai_addrlen) == -1) {
			close(sockfd6);
			perror("listener: bind");
			continue;
		}
	...
	addr_len6 = sizeof their_addr6;
	if ((numbytes6 = recvfrom(sockfd6, buf66, MAXBUFLEN-1 , 0,
		(struct sockaddr *)&their_addr6, &addr_len6)) == -1) {
		perror("recvfrom");
		exit(1);
	}

	//printf("listener: got packet from %s\n",
		inet_ntop(their_addr6.ss_family,
			get_in_addr((struct sockaddr *)&their_addr6),
			s6, sizeof s6);
	//printf("listener: packet is %d bytes long\n", numbytes6);
	buf66[numbytes6] = '\0';
	...
	printf("%s",buf66);
	printf("has been received from Store_2");
	//printf("listener: packet contains \"%s\"\n", buf66);

	/////////////////////////////////////////////////////////////////
    /*printf("Before Outlet vectors and Truck vectors");
	printf("o1");
	printf("%d",o1);
	printf("o2");
	printf("%d",o2);
...
for(j=0;j<(strlen(buffer31));j++)
   ...
   /* if (argc != 3) {
    ...
    hints7.ai_family = AF_UNSPEC;
    hints7.ai_socktype = SOCK_DGRAM;

    if ((rv7 = getaddrinfo("nunki.usc.edu", SERVERPORT11, &hints7, &servinfo7)) != 0) {
        fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv7));
        return 1;
    }
    ...
    if ((numbytes7 = sendto(sockfd7, buffer41, 20, 0,
             p7->ai_addr, p7->ai_addrlen)) == -1) {
        perror("talker: sendto");
        exit(1);
    }
	...
	inet_ntop(p7->ai_family, get_in_addr((struct sockaddr *)p7->ai_addr),
			s, sizeof s);
